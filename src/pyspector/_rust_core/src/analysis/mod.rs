...
pub fn run_analysis(context: AnalysisContext) -> Vec<Issue> {
    println!(
        "[*] Starting analysis with {} rules",
        context.ruleset.rules.len()
    );
    ...
    let python_issues: Vec<Issue> = context
        .py_files
        .par_iter()
        .flat_map(|py_file| {
            ...
            if let Some(ast) = &py_file.ast {
                let ast_findings = ast_analysis::scan_ast(
                    ast,
                    &py_file.file_path,
                    &py_file.content,
                    &context.ruleset,
    ...
    println!(
        "[+] {} issues from Python AST analysis",
        python_issues.len()
    );
    ...
    println!(
        "[+] Found {} issues from taint analysis",
        taint_issues.len()
    );
